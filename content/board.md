---
title: 邮件跟进清单
subtitle: Eric Tan · Inbox + Sent
updated: 2026-07-21
badge: incremental
priorities:
  - Sanaky：空运到货文件 — Invoice / PL / CQ / Test Report / Form E 草稿
  - Kimmy：PO26-025 全套附件 + 沟通邮件打包
  - EEMC 对账：核对阿姐明细 → 回 SOA（截至 6/30）
---

## todo | 你要处理

### Sanaky BSHV — 空运文件包
- priority: P0
- meta: Trung · 7/20 22:54 · 已收 Air bill · Sent 无回
- action: 发 **Invoice、PL、CQ、Test Report、Form E 草稿** 供核对（CQ/TR 归你）。

### EEMC PO26-025 — Kimmy 要资料
- priority: P0
- meta: 李倩 → 你 · 投产已 Fwd，附件包未回
- action: 把**所有附件 + 沟通邮件**整理发给她。

### EEMC 对账 / SOA（截至 2026-06-30）
- priority: P0
- meta: Hà 正式要 SOA · 阿姐已回海外/SG 往来
- action: 对明细，确认后回 Statement of Account。

### EVN UZF retrofit 交期
- priority: P1
- meta: Hoa 嫌 9/5 晚 · 你已 Noted 会再争取
- action: 跟工厂新提货日，有结果再回。

### Sanaky — 吸油管位置 1→2
- priority: P1
- meta: DETC 管系改位 · 你+Anthony+Kimmy
- action: 工程确认可否后回复客户。

### Bambang PX-360 图纸确认投产
- priority: P2
- meta: Kimmy 催 Yoeli 确认外形图
- action: 盯客户确认，确认后投产。

### HMSG Trading Process
- priority: P2
- meta: John → 全员
- action: 读附件流程，SG 属地按新规。

### PO25-535 继电器顶盖
- priority: P2
- meta: 曾问补发
- action: 跟有无补发 / 物流节点。

## done | 已推进

### PO26-025 已请 Kimmy ERP 投产 · 客户 Noted
- priority: OK

### 对账已发阿姐 · 阿姐已回海外/SG 明细
- priority: OK

### EVN 已回 Hoa（Noted / 争取提前）
- priority: OK

### 新加坡合同 CPF · 已 Noted
- priority: OK

### HMP26-428 空运 · 到货数据已出（Unitex/玲玲）
- priority: OK

## fyi | FYI

### HLG BTDA / HAVEC / MEE / DPTSC / Clough / SRG Audit
- priority: FYI
- action: Anthony / Kimmy / Yolanda 主跟。

### Gerrick：OLTC Product Approval 合规核对
- priority: FYI
- action: 找 Kimmy 做 full compliance check。

### Bambang PX-373 · 无法匹配合适 OLTC
- priority: FYI
- action: Kimmy 已拒供说明。

### HLG 协议 / Q2 佣金
- priority: FYI
- action: Kimmy / Anthony / Gerrick。
